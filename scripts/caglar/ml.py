...
class MLP(object):
    ...
    def __init__(self,
        ...
        self._preds = theano.function([x], self.clean_layers[-1].output)
    ...
    def compute_preds(self, x, batch_size=None):
        if batch_size:
            out = []
            n_batches = int(numpy.ceil(x.shape[0] / float(batch_size)))
            for n in range(n_batches):
                out.append(self._preds(x[n * batch_size : (n+1) * batch_size ]))
            return numpy.concatenate(out)
        else:
    ...
    def save(self):
        for i, layer in enumerate(self.layers):
            numpy.save(os.path.join(self.base_path, "W_%d.npy" % i), layer.W.get_value())
            numpy.save(os.path.join(self.base_path, "b_%d.npy" % i), layer.b.get_value())


    def load(self):
        for i, layer in enumerate(self.layers):
            layer.W.set_value(numpy.load(os.path.join(self.base_path, "W_%d.npy" % i)))
            layer.b.set_value(numpy.load(os.path.join(self.base_path, "b_%d.npy" % i)))
