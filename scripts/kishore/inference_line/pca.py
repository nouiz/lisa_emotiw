...
if __name__ == '__main__':
    import tables
    # data_l = np.load('/shared/data/Hollywood3D/l2_Samples_200k_Lx14x20x20.npy','r')
    
    # data_r = np.load('/shared/data/Hollywood3D/l2_Samples_200k_Rx14x20x20.npy','r')

    # data = np.concatenate((data_l,data_r),0)

    data = np.load('/shared/data/Hollywood3D/Samples_500k_2x10x16x16.npy')
    data = data.reshape(-1,2560)

    ncases = data.shape[0]
    
    h5file = tables.openFile(
        '/shared/data/Hollywood3D/layer1_inputdata_2x300.h5',
        mode='w', title='input data for layer1 (reduced dimensions)')
    preprocessing_params = np.load(
        '/shared/data/Hollywood3D/preprocessing_params.npz')
...
# if __name__ == '__main__':
#     import tables
#     data = np.load('/shared/data/Hollywood3D/hollywood_Cvninput_samples.npy')
    
#     data = data.reshape(-1,64,64).reshape(-1,64*64)

#     ncases = data.shape[0]
    
#     h5file = tables.openFile(
#         '/shared/data/Hollywood3D/hollywood_Cvninput_zca.h5',
#         mode='w', title='input data for cvnSAE')
#     preprocessing_params = np.load(
#         '/shared/data/Hollywood3D/preprocessing_params_zca.npz')
...
#                        'V', preprocessing_params['V'].astype(np.float32))
#     h5file.createCArray(h5file.root, 'data_white', tables.Float32Atom(),
#                         shape=(ncases, data.shape[1]),
#                         chunkshape=(100000, data.shape[1]))
#     batchsize = 50000
#     nbatches = (ncases - 1) / batchsize + 1
#     for bidx in range(nbatches):
#         print "processing batch %d of %d" % (bidx+1, nbatches)
#         start = bidx * batchsize
#         end = min((bidx + 1) * batchsize, ncases)
#         h5file.root.data_white[start:end] = zca_whiten(
#             data[start:end], preprocessing_params['V'].astype(np.float32),
#             batchsize=10000, use_gpu=True, verbose=True)
#         print "flushing..."
#         h5file.flush()
