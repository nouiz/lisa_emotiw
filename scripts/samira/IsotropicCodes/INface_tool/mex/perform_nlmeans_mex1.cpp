...
#define Tm_(a,b) access(Tm,a,b,0)
...
inline double weight_gaussian(double x,double Ts)
{
	// x is the distance
	return exp(-(x*x)/(2*Ts*Ts))/Ts;
}
...
{		
	...
	for( int j1=j_min; j1<=j_max; ++j1 )
	{
		...
		//double tmpi = Tm_(i1,j1);
		//ww = exp(-(ww*ww)/(2*tmpi*tmpi))/tmpi;
		ww = weight_func(ww,Tm_(i,j));
		w_sum += ww;
		w_(i1,j1) = ww;
		}
		else
...
{ 
	...
	// -- input 13 : Tm gaussian weight for each pixel
	if( nrhs>=13 ) 
	{	get_dimensions( prhs[2], m1,n1,k1 );
		if( na!=n1 || ma!=m1 || k!=k1  )
		mexErrMsgTxt("Tm should be of same size as Ma.");
		Tm = mxGetPr(prhs[12]);
	}
